...
pub enum CommandResult {
    ...
    Message(String),
    /// Command produced a message AND pre-fills the input box with the next step.
    /// Used for interactive wizard flows (e.g., /providers step-by-step setup).
    /// The `prefill` string is placed verbatim into the prompt input — the user
    /// only has to paste/type the remaining value and press Enter.
    MessageWithPrefill {
        message: String,
        prefill: String,
    },
