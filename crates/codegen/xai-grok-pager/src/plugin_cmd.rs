//! `fusion plugin` CLI subcommand — manage plugins and marketplace sources.
//!
...
/// Typed entry for `fusion plugin list --json`. The `status` field acts as a
...
/// Typed entry for `fusion plugin marketplace list --json`.
#[derive(Serialize)]
...
pub enum PluginCommand {
    ...
    Uninstall {
        /// Plugin name (as shown by `fusion plugin list`).
        name: String,
...
fn cmd_list(json: bool, available: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("No plugins installed. Run `fusion plugin install --help` to get started.");
    } else {
            ...
            if outcome.already_installed {
                ...
                println!(
                    "Plugin \"{}\" is already installed from {}. \
                     Run `fusion plugin update {}` to update it.",
    ...
    if registry.find_plugin(name).is_none() {
        bail!(
            "Plugin \"{name}\" not found.\n\
               Run `fusion plugin list` to see installed plugins."
        );
    }
...
fn cmd_tag(path: &str, push: bool, force: bool, dry_run: bool) -> Result<()> {
    ...
    let version = match load_manifest(&root) {
        Ok(ManifestLoadResult::Found(m)) => m.version.ok_or_else(|| {
            anyhow::anyhow!(
                "No `version` field in plugin.json. Set a version to use `fusion plugin tag`."
            )
        })?,
    ...
    if json {
        ...
        println!(
            "No marketplace sources configured.\n\
             Run `fusion plugin marketplace add --help` to get started."
        );
    } else {
        for s in sources {
            let id = match &s.kind {
