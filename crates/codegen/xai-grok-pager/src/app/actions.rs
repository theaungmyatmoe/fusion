...
pub enum Action {
    ...
    /// Open grok.com in the browser for Fusion subscription upsell.
    OpenSupergrokUrl,
    ...
    /// Open the provider setup wizard overlay for a given provider name.
    /// Triggered by `/providers cloudflare`, `/providers openai`, etc.
    OpenProviderWizard {
        provider: String,
    },
    /// User completed the provider wizard — save credentials and show success.
    ProviderWizardSubmit {
        provider: String,
        values: Vec<String>,
    },
...
///
/// Both variants write to `~/.fusion/config.toml` and route ACP
...
pub enum Effect {
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.fusion/config.toml`.
    ...
    /// Persist a typed setting to `~/.fusion/config.toml`. On failure,
    ...
    /// registry (`~/.fusion/active_sessions.json`).
