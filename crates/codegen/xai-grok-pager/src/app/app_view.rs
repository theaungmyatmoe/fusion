...
/// State for the provider setup wizard overlay (triggered by `/providers <name>`).
#[derive(Debug, Clone)]
pub struct ProviderWizardState {
    /// Provider name: "cloudflare", "openai", "deepseek", "xai".
    pub provider: String,
    /// One entry per field (e.g. ["Account ID", "API Token"]).
    labels: Vec<&'static str>,
    /// Marks which field indices contain secret data (shown as ••••).
    secret_fields: Vec<bool>,
    /// Current typed value for each field.
...
impl ProviderWizardState {
    /// Create a wizard for the given provider name.
    pub fn for_provider(provider: &str) -> Self {
        let (labels, secret_fields): (Vec<&'static str>, Vec<bool>) =
            match provider.to_lowercase().as_str() {
                "cloudflare" => (
                    vec!["Account ID", "API Token"],
                    vec![false, true],
                ),
                _ => (vec!["API Key"], vec![true]),
            };
        let n = labels.len();
        Self {
    ...
    pub fn title(&self) -> String {
        match self.provider.as_str() {
            ...
            "openai"     => "OpenAI Setup".to_string(),
            "deepseek"   => "DeepSeek Setup".to_string(),
            "xai"        => "xAI Setup".to_string(),
            other        => format!("{} Setup", other),
        }
    }

    /// Number of fields.
    pub fn fields(&self) -> usize { self.labels.len() }

    /// Labels for each field in order.
    pub fn field_labels(&self) -> &[&'static str] { &self.labels }

    /// Whether field `i` is a secret (masked) input.
    pub fn is_secret_field(&self, i: usize) -> bool {
        self.secret_fields.get(i).copied().unwrap_or(false)
    }

    /// Handle a key event. Returns the wizard outcome.
    pub fn handle_key(&mut self, key: &crossterm::event::KeyEvent) -> ProviderWizardOutcome {
        use crossterm::event::{KeyCode, KeyModifiers};
        ...
        {
            return match key.code {
                KeyCode::Char('c' | 'd') => ProviderWizardOutcome::Cancelled,
                _ => ProviderWizardOutcome::Unchanged,
            };
        }

        match key.code {
            KeyCode::Esc => ProviderWizardOutcome::Cancelled,

            // Tab / Shift+Tab — move between fields
            KeyCode::Tab => {
                if self.active_field + 1 < self.fields() {
                    self.active_field += 1;
                    ProviderWizardOutcome::Changed
                } else {
                    ProviderWizardOutcome::Unchanged
                }
            }
            KeyCode::BackTab => {
            ...
            KeyCode::Enter => {
                if self.active_field + 1 < self.fields() {
                    ...
                    ProviderWizardOutcome::Submitted(values)
                }
            }

            KeyCode::Backspace => {
                if self.inputs[self.active_field].pop().is_some() {
                    ProviderWizardOutcome::Changed
                } else {
                    ProviderWizardOutcome::Unchanged
                }
            ...
            KeyCode::Char(c) => {
                if self.inputs[self.active_field].len() < 512 {
                    self.inputs[self.active_field].push(c);
                }
                ProviderWizardOutcome::Changed
            }
...
/// Outcome of handling input in the provider wizard.
#[derive(Debug)]
pub enum ProviderWizardOutcome {
    /// User completed all fields and pressed Enter on the last one.
