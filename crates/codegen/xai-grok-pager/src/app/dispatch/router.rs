...
pub(crate) fn dispatch(action: Action, app: &mut AppView) -> Vec<Effect> {
    ...
    let effects = match action {
        ...
        Action::OpenProviderWizard { provider } => {
            if let Some(agent) = get_active_agent_mut(app) {
                agent.provider_wizard =
                    Some(crate::app::app_view::ProviderWizardState::for_provider(&provider));
            }
            vec![]
        }
        Action::ProviderWizardSubmit { provider, values } => {
            use crate::slash::commands::providers::apply_wizard_result;
            use crate::scrollback::RenderBlock;
            let msg = match apply_wizard_result(&provider, &values) {
                Ok(m) => m,
                Err(e) => format!("❌ Failed to save {} credentials: {}", provider, e),
            };
            if let Some(agent) = get_active_agent_mut(app) {
