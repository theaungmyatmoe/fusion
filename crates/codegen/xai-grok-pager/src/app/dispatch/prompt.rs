...
pub(super) fn dispatch_send_prompt_inner(
    ...
    if !literal && trimmed.starts_with('/') {
        ...
        match exec_result {
            ...
            CommandResult::MessageWithPrefill { message, prefill } => {
                // Show the step message in scrollback, then pre-fill the input
                // box with the next wizard step so the user only needs to
                // append their value and press Enter.
                agent.scrollback.push_block(RenderBlock::system(message));
                agent.prompt.set_text(&prefill);
                return vec![];
            }
