...
fn print_exit_resume_hint(session_id: &str, minimal: bool, w: &mut impl Write) {
    ...
    if minimal {
        let _ = writeln!(w, "  fusion --minimal --resume {session_id}");
    } else {
        let _ = writeln!(w, "  fusion --resume {session_id}");
    }
}
...
mod tests {
    ...
    fn terminal_title_strips_control_characters() {
        assert_eq!(
            ...
            "evil]52;c;payloadtitle - fusion"
        );
        assert_eq!(terminal_title_string("\x07\x1b\x00"), "fusion");
        assert_eq!(terminal_title_string(""), "fusion");
        assert_eq!(terminal_title_string("My chat"), "My chat - fusion");
    }
    #[test]
    ...
    fn print_exit_resume_hint_includes_minimal_flag() {
        ...
        assert_eq!(
            String::from_utf8(buf).unwrap(),
            "\nResume this session with:\n  fusion --minimal --resume sess-abc\n"
        );
    }
    #[test]
