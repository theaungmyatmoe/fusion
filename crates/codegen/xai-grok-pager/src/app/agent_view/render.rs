...
impl AgentView {
    ...
    pub fn draw(
        ...
        let prompt_style = PromptStyle {
            ...
            prefix_override: if let Some(ref wizard) = self.provider_wizard {
                let label = wizard.field_labels()[wizard.active_field];
                Some((format!("{}: ", label), theme.accent_user))
            } else if let Some((p, c)) = self.prompt_input_mode.prefix_override(&theme) {
                Some((p.to_string(), c))
            } else if casual_commenting
                || self
                    .plan_approval_view
        ...
        let info = if self.provider_wizard.is_some() {
            PromptInfo {
                model_name: "Fusion Setup Wizard",
                flags: &[],
                multiline: false,
                usage_warning: None,
                usage_warning_critical: false,
            }
        } else if let Some(label) = self.prompt_input_mode.prompt_info_override() {
            PromptInfo {
                model_name: label,
                flags: &[],
                multiline: false,
