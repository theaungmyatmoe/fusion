...
impl AgentView {
    ...
    pub(in crate::app) fn handle_prompt_key(
        ...
        // ── Provider Setup Wizard Intercept ─────────────────────────────────────
        if let Some(ref mut wizard) = self.provider_wizard {
            use crate::views::prompt_widget::PromptEvent;
            match key.code {
                KeyCode::Esc => {
                ...
                KeyCode::Tab => {
                    if wizard.fields() > 1 {
                        ...
                        wizard.inputs[wizard.active_field] = text;
                        wizard.active_field = (wizard.active_field + 1) % wizard.fields();
                        self.prompt.set_text(&wizard.inputs[wizard.active_field]);
                        return InputOutcome::Changed;
                    }
                }
                KeyCode::BackTab => {
                    if wizard.fields() > 1 {
                        let text = self.prompt.text().trim().to_string();
                        wizard.inputs[wizard.active_field] = text;
                        wizard.active_field = (wizard.active_field + wizard.fields() - 1) % wizard.fields();
                        self.prompt.set_text(&wizard.inputs[wizard.active_field]);
                        return InputOutcome::Changed;
                    }
                }
                KeyCode::Enter => {
                    ...
                    self.prompt.set_text("");
                    wizard.inputs[wizard.active_field] = text;
                    if wizard.active_field + 1 < wizard.fields() {
                        wizard.active_field += 1;
                        self.prompt.set_text(&wizard.inputs[wizard.active_field]);
                        return InputOutcome::Changed;
                    } else {
                        let provider = wizard.provider.clone();
                        let values = wizard.inputs.clone();
                        self.provider_wizard = None;
                        return InputOutcome::Action(Action::ProviderWizardSubmit { provider, values });
                    }
                }
                _ => {
                    match self.prompt.handle_key(key) {
                        PromptEvent::Edited => return InputOutcome::Changed,
                        PromptEvent::Ignored => return InputOutcome::Unchanged,
                    }
                }
            }
        }
