...
pub struct PromptStyle {
    ...
    pub prefix_override: Option<(String, ratatui::style::Color)>,
...
impl PromptWidget {
    ...
    pub fn desired_height(
        ...
        let prefix_w = if style.show_prefix {
            if let Some((ref p, _)) = style.prefix_override {
                use unicode_width::UnicodeWidthStr;
                p.width() as u16
            } else {
                PREFIX_WIDTH
        ...
        if style.show_prefix {
            let (prefix_str, accent_color) =
                if self.history_search.is_active() && !self.history_search.is_browse() {
                    ...
                    ("? ".to_string(), theme.accent_user)
                } else if let Some((ref p, c)) = style.prefix_override {
                    // Caller override (e.g., bash mode `! ` in yellow).
                    (p.clone(), c)
                } else {
                    (crate::glyphs::prompt_arrow().to_string(), style.accent_color(&theme))
                };
            use unicode_width::UnicodeWidthStr;
            prefix_w = prefix_str.width() as u16;
            if text_area_rect.width > prefix_w {
                buf.set_string(
                    text_area_rect.x,
                    text_area_rect.y,
                    &prefix_str,
