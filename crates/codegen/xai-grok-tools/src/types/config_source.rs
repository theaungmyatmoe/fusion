...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.fusion/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.fusion/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.grok/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.fusion/ or ~/.claude/.
    User { path: PathBuf },
