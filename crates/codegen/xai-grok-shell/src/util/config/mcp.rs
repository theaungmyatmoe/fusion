...
/// Project-scoped `.grok/config.toml` entries override global `~/.fusion/config.toml`
...
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.fusion/config.toml`
...
///
/// Uses the top-level `disabled_mcp_servers` array in `~/.fusion/config.toml`.
...
/// Upsert an MCP server entry in `~/.fusion/config.toml`.
///
...
/// Delete an MCP server entry from `~/.fusion/config.toml`.
///
...
/// Path to the user-level config file (`~/.fusion/config.toml`).
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.fusion/skills", "~/.fusion/skills/special/SKILL.md"]
ignore = ["~/.fusion/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.fusion/skills", "~/.fusion/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.fusion/skills/noisy/SKILL.md"]);
    }
