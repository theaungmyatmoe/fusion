...
pub struct LeaderCapabilities {
    ...
    /// (driven by `fusion update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `fusion update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `fusion update` just installed; the leader uses
