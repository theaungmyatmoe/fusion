...
async fn run_auth_flow_inner(
    ...
    anyhow::bail!(
        "No OAuth2 configuration available. Run `fusion login` to authenticate, or contact your administrator if you use enterprise SSO."
    )
}
...
/// Unified `fusion login` handler for CLI entry points (tui, pager).
///
...
pub async fn run_cli_login(
    ...
    // supports the device flow. Without this guard, `fusion login` on an
    ...
    // config was actually applied; `fusion setup` reports the no-config case.
...
mod tests {
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `fusion login` must use loopback, not device —
    ...
    /// CLI `fusion login` passes `on_stderr=None`; stderr must be inherited so
