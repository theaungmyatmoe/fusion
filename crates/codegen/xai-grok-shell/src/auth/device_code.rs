...
pub enum DeviceCodeError {
    #[error(
        "Device-code login is not available for this deployment. \
         Try `fusion login` or set XAI_API_KEY instead."
    )]
    NotEnabled,
    #[error(transparent)]
...
///
/// On success, persists credentials to `~/.fusion/auth.json` and returns
...
pub async fn complete_device_code_login(
    ...
    loop {
        ...
        if tokio::time::Instant::now() > deadline {
            anyhow::bail!("Device code expired. Run `fusion login --device-auth` again.");
        }
