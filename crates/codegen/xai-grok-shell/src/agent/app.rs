...
pub async fn run_stdio_agent(
    ...
    // Log the client that launched us (set by grok-desktop when spawning `fusion agent stdio`).
    ...
    let result = local_set
        .run_until(async move {
            ...
            // `fusion agent stdio` is a local/interactive entrypoint (spawned by
...
async fn run_headless_inner(
    ...
    register_fs_watch_runtime();
    xai_grok_telemetry::unified_log::set_version(xai_grok_version::VERSION);
    // `fusion agent [headless]` serves non-TUI automation; stamp proxy requests
    // as headless. IDE-facing `fusion agent stdio` stays interactive.
    ...
    const HEADLESS_NO_SESSION: &str = "Headless mode requires a grok.com session. \
        Run `fusion login` to sign in, or use `fusion agent stdio` for API-key access.";
    ...
    let on_first_connect: Box<dyn FnOnce() + Send + 'static> = Box::new(move || {
        if !did_browser_flow && !no_browser {
            ...
            eprintln!();
            eprintln!(
                "Open Fusion: {} (press Enter to open in browser)",
                grok_code_url
            );
            eprintln!();
...
///
/// * `relay_on_demand == false` (default — explicit `fusion agent leader`
...
pub async fn run_leader(
    ...
    local_set
        .run_until(async move {
            ...
            // `fusion agent leader` (devbox / systemd) has no local IPC clients
                ...
                while let Some(update) = config_update_rx.recv().await {
                    match update {
                        ...
                        ConfigUpdate::ModelsCacheChanged => {
                            // External write to ~/.fusion/models_cache.json
...
mod tests {
    ...
    /// `fusion agent leader` (devbox/systemd — no local IPC clients,
        ...
        local
            .run_until(async {
                let slot = spawn_leader_relay(
                    config,
                    false, // eager: explicit `fusion agent leader` invocation
                    demand_rx,
                    ws_to_agent_tx,
                    agent_to_ws_tx.clone(),
                    cancel.clone(),
