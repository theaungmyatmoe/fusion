...
/// files (TUI tick vs `fusion login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `fusion setup`; runs retries to completion.
    Standard,
...
/// Fetch managed config + requirements and write to `~/.fusion/`, trying the
...
struct SyncOutcome {
    ...
    /// `fusion setup` doesn't misreport a lock skip as "no config".
    served: bool,
...
/// Post-login hook for `fusion login` and the ACP/TUI authenticate flow: clear any
...
/// Whether a credential exists that `fusion setup` could install config for.
...
/// `fusion setup`); ceasing to serve `fail_closed` rolls back.
...
/// Outcome of the `fusion setup` sync. The caller renders it — CLI presentation
/// and exit codes stay out of the library.
#[derive(Debug)]
pub enum SetupOutcome {
    /// Config was written to `~/.fusion`.
    Installed,
...
/// exactly as `fusion setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `fusion setup` would install them.
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `fusion setup` would refuse
...
/// Run the `fusion setup` sync for the current principal. The caller must check
