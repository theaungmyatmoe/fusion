...
fn print_serve_startup_info(bind_addr: SocketAddr, secret: &str) {
    eprintln!();
    eprintln!("   Fusion agent server starting...");
    eprintln!();
...
async fn run_setup_command(json: bool) {
    ...
    if !managed_config::has_principal() {
        ...
        eprintln!();
        eprintln!("To install managed configuration, sign in with a team using `fusion login`,");
        ...
        eprintln!();
        eprintln!(
            "If you don't have a deployment key, contact your organization's Fusion administrator."
        );
        std::process::exit(1);
    }
    if json {
...
async fn run_workspace_mgmt(args: WorkspaceMgmtArgs) -> Result<()> {
    ...
    match workspace_command_gate(env_override, remote_settings.as_ref()) {
        ...
        WorkspaceGate::Disabled => {
            anyhow::bail!(
                "`fusion workspace` is not enabled for this account \
             (gated by a server-side feature flag that is currently off)."
            )
        }
        WorkspaceGate::Unknown => {
            anyhow::bail!(
                "Could not load your settings for `fusion workspace`. Check your \
             network connection (run `fusion login` if you are signed out), then \
             try again."
            )
        }
    }
    ...
    .map_err(|e| {
        anyhow::anyhow!(
            "no running leader for this environment ({e}). \
             Start a fusion session, or run `fusion workspace start`."
        )
    })
}
...
async fn workspace_start(
    ...
    ensure_authenticated(
        ...
        false,
        Some("No cached credentials found. Run `fusion login` first."),
    )
    .await?;
...
fn main() {
    ...
    if let Err(e) = xai_grok_config::validate_requirements() {
        eprintln!("Couldn't start Fusion: {e}");
        eprintln!();
        eprintln!(
            "Update Fusion to a version the policy allows, or ask your administrator \
    ...
    if xai_grok_shell::util::config::load_crash_handler_enabled_sync() {
        ...
        if let Some(report) = xai_crash_handler::check_previous_crash(&crash_dir) {
            eprintln!("Fusion crashed during your last session.");
    ...
    match result {
        Ok(true) => {
            ...
            if finish_update_on_exit(adopted, &update_config).await {
                ...
                eprintln!("Update did not complete. Run `fusion update` to retry.");
            }
            Ok(())
        }
        Ok(false) => Ok(()),
