...
pub fn grok_home() -> PathBuf {
    GROK_HOME
        .get_or_init(|| {
            let grok_home = if let Ok(v) = std::env::var("FUSION_HOME") {
                PathBuf::from(v)
...
pub fn user_grok_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable = std::env::var_os("FUSION_HOME").is_some() || std::env::var_os("GROK_HOME").is_some() || std::env::home_dir().is_some();
    resolvable.then(grok_home)
}

/// Canonical grok application path: `$GROK_HOME/bin/fusion` (Unix) or `fusion.exe` (Windows).
pub fn grok_application() -> PathBuf {
    let name = if cfg!(windows) { "fusion.exe" } else { "fusion" };
    grok_home().join("bin").join(name)
}

/// System-wide config directory: `/etc/fusion/` (Unix), `None` on Windows.
pub fn system_config_dir() -> Option<PathBuf> {
    if cfg!(unix) {
        if let Ok(prefix) = std::env::var("PREFIX") {
            Some(PathBuf::from(prefix).join("etc").join("fusion"))
        } else {
            Some(PathBuf::from("/etc/fusion"))
        }
    } else {
        None
    }
